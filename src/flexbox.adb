   ...
   begin
      ...
      for I in 1 .. Container.Item_Count loop
         if Free_Space > 0 and Total_Grow > 0.0 then
            Container.Items(I).Computed_Size := Container.Items(I).Flex_Basis + Integer (Float (Free_Space) * (Container.Items(I).Flex_Grow / Total_Grow));
         elsif Free_Space < 0 and Total_Shrink > 0.0 then
            Container.Items(I).Computed_Size  := Integer'Max(1, Container.Items(I).Flex_Basis + Integer(Float(Free_Space) * (Container.Items(I).Flex_Shrink / Total_Shrink)));
         else
      ...
      for I in 1 .. Container.Item_Count loop
         case Container.Align is
            when Flex_Start | Center | Stretch =>
               Container.Items(I).Cross_Size := (if Container.Direction = Row then Container.Height else Container.Width);
         end case;
      end loop;
      ...
      declare
         ...
         Gap    : Integer := 0;
      begin
         -- Only apply justification logic if we have extra space and aren't growing to fill it
         ...
         for I in 1 .. Container.Item_Count loop
            if Container.Direction = Row then
               ...
               case Container.Align is
                  when Flex_Start => Container.Items(I).Position_Y := 0;
                  when Center     => Container.Items(I).Position_Y := (Container.Height - Container.Items(I).Cross_Size) / 2;
                  when Stretch    => Container.Items(I).Position_Y := 0;
               end case;
               Cursor := Cursor + Container.Items(I).Computed_Size + Gap;
            else
               Container.Items(I).Position_Y := Cursor;
               case Container.Align is
                  when Flex_Start => Container.Items(I).Position_X := 0;
                  when Center     => Container.Items(I).Position_X := (Container.Width - Container.Items(I).Cross_Size) / 2;
                  when Stretch    => Container.Items(I).Position_X := 0;
               end case;
               Cursor := Cursor + Container.Items(I).Computed_Size + Gap;
            end if;
         end loop;
      ...
      -- FIX: Only fill the remaining pixel gap IF the items were actually supposed to grow.
      -- If Total_Grow is 0, we shouldn't force the last item to jump to the edge.
      if Total_Grow > 0.0 then
          declare
             Used_Space : Integer := 0;
             Target_Space : Integer := (if Container.Direction = Row then Container.Width else Container.Height);
          begin
             for I in 1 .. Container.Item_Count loop
             ...
             if Used_Space /= Target_Space then
                Container.Items(Container.Item_Count).Computed_Size :=
                   Container.Items(Container.Item_Count).Computed_Size + (Target_Space - Used_Space);
             end if;
          end;
      end if;
