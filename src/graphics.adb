...
package body Graphics is
   ...
   -- If GetStdHandle fails, it returns (HANDLE)(-1), i.e. all bits set
   ...
   exception
      when Program_Error => null; -- Console API unavailable (e.g. WSL)
   ...
   procedure Set_Cursor_Visible (Visible : Boolean) is
      ...
      pragma Warnings (Off, Res);
   begin
      -- Validate handle is not invalid AND is a real console handle
      -- GetConsoleMode fails (returns 0) for pipe/file/WSL handles
      if H /= INVALID_HANDLE_VALUE
        and then GetConsoleMode (H, Mode'Access) /= 0
      then
   ...
   exception
      when Program_Error =>
         -- Console API unavailable (e.g. WSL), fall back to ANSI
         if Visible then
