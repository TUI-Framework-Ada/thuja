...
--    Search_Component_IDs: Component_ID_Vector.Vector := ["Component1", "Component2"];
--    Matched_Entities: Entity_ID_Vector.Vector := Entity_List.Get_Entities_Matching(Search_Component_IDs);
...
-- end ExampleSystem;
   ...
   procedure TextRenderSystem(Entity_List: Entities) is
      Search_Component_IDs: Component_ID_Vector.Vector := ["WidgetComponent", "TextComponent"];
      Matched_Entities: Entity_ID_Vector.Vector := Entity_List.Get_Entities_Matching(Search_Component_IDs);
      Component_List: Components_Access;
   ...
   begin
      for Entity_ID of Matched_Entities loop
         Component_List := Entity_List.Get_Entity_Components(Entity_ID);
         Widget_C := WidgetComponent(Component_List.Get_Component("WidgetComponent"));
         Text_C := TextComponent(Component_List.Get_Component("TextComponent"));
         Text := Text_C.Text;
   ...
   procedure BufferCopySystem(Entity_List: Entities) is
      procedure RecursiveBufferCopy(Framebuffer: Buffer_T; Parent: WidgetComponent) is
         Child_Component_List: Components_Access;
         Child_Widget: WidgetComponent;
         Parent_Pixel: Pixel;
         FB_Pixel: Pixel;
      begin
         -- For each pixel of Render_Buffer, only within the bounds of the widget
         ...
         for Pos_W in Positive'First .. Parent.Size_Width loop
            for Pos_H in Positive'First .. Parent.Size_Height loop
               ...
               FB_Pixel := Framebuffer.Get_Pixel(Parent.Position_X + Pos_W - 1, Parent.Position_Y + Pos_H - 1);
...
--             FB_Pixel.Background_Color := Parent_Pixel.Background_Color;
         ...
         for Child_Entity_ID of Parent.Children loop
            ...
            Child_Component_List := Entity_List.Get_Entity_Components(Child_Entity_ID);
            Child_Widget := WidgetComponent(Child_Component_List.Get_Component("WidgetComponent"));
      ...
      Matched_RIs: Entity_ID_Vector.Vector := Entity_List.Get_Entities_Matching(RI_Component_IDs);
      Matched_Roots: Entity_ID_Vector.Vector := Entity_List.Get_Entities_Matching(Root_Component_IDs);
   ...
   begin
      ...
      for RI_Entity_ID of Matched_RIs loop
         ...
         RenderInfo_C := RenderInfo(RI_Components.Get_Component("RenderInfo"));
         -- For each root
         for R_Entity_ID of Matched_Roots loop
            Root_Components := Entity_List.Get_Entity_Components(R_Entity_ID);
            Root := WidgetComponent(Root_Components.Get_Component("WidgetComponent"));
...
--           & Trim(P.Background_Color.Blue'Image) & "m");
      function Bold(P: Pixel) return String is (CSI & (if P.Is_Bold then "1m" else "0m"));
--    function Format(P: Pixel) return String is (FG(P) & BG(P) & Bold(P));
      function Format(P: Pixel) return String is (FG(P) & Bold(P));
      function Move(Row: Positive; Col: Positive) return String is
        (CSI & Trim(Row'Image) & ";" & Trim(Col'Image) & "H");
      function ConvertWW(P: Pixel; Row: Positive; Col: Positive) return Wide_Wide_String is
        (Move(Row, Col) & Ada.Characters.Conversions.To_Wide_Wide_String(Format(P)) & Wide_Wide_Character(P.Character));
      RESET: constant String := CSI & "0m";

      -- Real stuff begins
      Search_Components: Component_ID_Vector.Vector := ["RenderInfo"];
      Matched_Entities: Entity_ID_Vector.Vector := Entity_List.Get_Entities_Matching(Search_Components);
      ...
      for Entity_ID of Matched_Entities loop
         ...
         for Y in 1 .. RI.Terminal_Height loop
            for X in 1 .. RI.Terminal_Width loop
               if RI.Framebuffer.Get_Pixel(X, Y) /= RI.Backbuffer.Get_Pixel(X, Y) then
                  -- Draw to terminal
                  Ada.Wide_Wide_Text_IO.Put(ConvertWW(RI.Framebuffer.Get_Pixel(X, Y), Y, X));
                  -- Fetch buffer pixels
